import { Button, DatePicker, Form, Input, Modal, /*Radio,*/ Select/*, Steps*/ ,InputNumber,Row,Col,Icon,Divider,message} from 'antd';
import React, { Component } from 'react';
import moment from 'moment';
import { FormComponentProps } from 'antd/es/form';

import { ListItemDataType/*,MemberSelect*/,CustomerSelect,Member,AccountExportItem,FiscalYearItem,supportType,projectRunStatus } from '../../data.d';
...
class ModifyForm extends Component<ModifyFormProps, ModifyState> {
  ...
  constructor(props: ModifyFormProps) {
    ...
    const tempPlanBudget = (typeof(props.values.plan_working_hours)=="undefined"?0:props.values.plan_working_hours )
                      * (typeof(props.values.hour_rate)=="undefined"?0:props.values.hour_rate);

    const planBudgetInitial = props.values.plan_budget;
    const arrAccountVol = typeof(props.values.accounts)=="undefined"?[]:props.values.accounts.accountVol;
    const arrPercentage = this.getPercentageArrInitial(arrAccountVol,planBudgetInitial);
    const arrAccountName = typeof(props.values.accounts)=="undefined"?[]:props.values.accounts.accountName;
    const arrFiscalYear = typeof(props.values.accounts)=="undefined"?[]:props.values.accounts.fiscalYear;
    this.state = {
      formVals: {
  ...
  onPlanWorkingHourChange = (event) =>{//debugger
  ...
  onHourRateChange = (value) =>{//debugger
    const planWorkingHour = this.props.form.getFieldValue("plan_working_hours");
    //const hourRate = this.props.form.getFieldValue("hour_rate");
  ...
  onBudgetChange = (value,k) => {//debugger
    const { form } = this.props;
  ...
  renderContent = ( formVals: FormValsType) => {
     //debugger
    const { form,apcList,customerList,bdList,pssList,salesList ,supportTypeList,projectRunStatusList} = this.props;
  ...
  const formItemLayout = {
    ...
    const accountOptions = this.props.accountList.map(d => <Option key={d.account_id}>{d.account_name}</Option>);
    const fiscalOptions = this.props.fiscalList.map(d => <Option key={d.fiscal_id}>{d.fiscal_year}</Option>);
    const supportTypeOptions = this.props.supportTypeList.map(d => <Option key={d.support_type_id}>{d.support_type_name}</Option>);
    //debugger
    const projectRunStatusOptions = this.props.projectRunStatusList.map(d => <Option key={d.project_run_status_id}>{d.project_run_status_name}</Option>);


    const { getFieldDecorator, getFieldValue } = this.props.form;
    //debugger
    getFieldDecorator('keys', { initialValue: this.props.values.accounts==null?[]:this.props.values.accounts.keys });
    ...
    const formItems = keys.map((k, index) => (
      <Row gutter={12}>
    <Col span={8}>
                <FormItem label={formatMessage({ id: 'project.ModifyForm.AccountBudget.AccountName' })}
                  required={false}
                  key={'aN_'+k}
                ...
                >
                  {getFieldDecorator('accountName['+k+']', {
                    ...
                    <Select showSearch optionFilterProp="children" placeholder={formatMessage({ id: 'project.ModifyForm.AccountBudget.AccountName.PlaceHolder' })} /*style={{width: '100%',border:'solid 1px #000' }}*/>
                                {accountOptions}
                    </Select>,
                  )}
                  </FormItem>
      ...
      <Col span={4}>
                  <FormItem label={formatMessage({ id: 'project.ModifyForm.AccountBudget.FiscalYear' })}
                  required={false}
                  key={'fY_'+k}
                >
                  {getFieldDecorator('fiscalYear['+k+']', {
                    ...
                    <Select showSearch  optionFilterProp="children" placeholder={formatMessage({ id: 'project.ModifyForm.AccountBudget.FiscalYear.PlaceHolder' })} /*style={{width: '100%',border:'solid 1px #000' }}*/>
                                {fiscalOptions}
                    </Select>,
                  )}
                  </FormItem>
        ...
        <Col span={6}>
                <FormItem label={formatMessage({ id: 'project.ModifyForm.AccountBudget.Budget' })}
                required={false}
                key={'aV_'+k}
              >
                {getFieldDecorator('accountVol['+k+']', {
                  ...
                  <InputNumber       
                          ...
                          step={500}
                          min={0}
                          onChange={(value)=>{this.onBudgetChange(value,k)}}
                          />
                )}
              </FormItem>
        </Col>
        <Col span={4}>
                <FormItem label={formatMessage({ id: 'project.ModifyForm.AccountBudget.Percentage' })}
                   required={false}
                  key={'aP_'+k}
                  //style={{border:'solid 1px #f00'}}
                >
    ...
    return (
      <Form>
        <Divider>{formatMessage({ id: 'project.ModifyForm.Divider.ProjectDetails' })}</Divider>
         <Row gutter={0}>
          <Col span={0}>
              <FormItem key="project_id" {...this.formLayout} label={"Project Id"} style={{display:"none"}}>
           ...
           <Col span={12}>
              <FormItem key="project_code" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.ProjectCode' })} /*style={{display:"none"}}*/>
              {form.getFieldDecorator('project_code', {
                initialValue: formVals.project_code,
              })(<Input prefix={"NO."} disabled={true} placeholder={formatMessage({ id: 'project.SearchInputTips' })} style={{textAlign:"center"}}/>)}
              </FormItem>
            </Col>
            <Col span={12}>
              <FormItem key="project_name" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.ProjectName' })}>
                  {form.getFieldDecorator('project_name', {
                    rules: [{ required: true, message: formatMessage({ id: 'project.NewCreateForm.Column.ProjectName.RulesMessage' }), min: 1 }],
          ...
          <Row gutter={0}>
            <Col span={12}>
                <FormItem key="customer_id" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.CustomerId' })}>
                {form.getFieldDecorator('customer_id', {
                  initialValue: formVals.customer_id + "",
                  rules: [{ required: true,message:formatMessage({ id: 'project.NewCreateForm.Column.CustomerName.RulesMessage' })}],
                })(
                  <Select showSearch placeholder={formatMessage({ id: 'project.SearchSelectTips' })} optionFilterProp="children" style={{width: '100%' }}>
                    {cusOptions}
                  </Select>,
                )}
              </FormItem>
            ...
            <Col span={12}>
                <FormItem key="owner" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.Owner' })}>
                {form.getFieldDecorator('owner', {
                  initialValue: formVals.owner_id + "",
                  rules: [{ required: true,message:formatMessage({ id: 'project.NewCreateForm.Column.Owner.RulesMessage' })}],
                })(
                  <Select showSearch placeholder={formatMessage({ id: 'project.SearchSelectTips' })} optionFilterProp="children" style={{width: '100%' }}>
          ...
          <Row gutter={0}>
              <Col span={12}>
              <FormItem key="status" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.Status' })}>
                  {form.getFieldDecorator('status' , {initialValue: formVals.status_id + "",})(
                        <Select placeholder={formatMessage({ id: 'project.SearchSelectTips' })} disabled={true} style={{width: '100%' }}>
              ...
              <Col span={12}>
              <FormItem key="support_type" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.SupportType' })}>
                  {form.getFieldDecorator('support_type' , {initialValue: formVals.support_type_id+"",})(
                        <Select placeholder={formatMessage({ id: 'project.SearchSelectTips' })} style={{width: '100%' }}>
                        {supportTypeOptions}
                        </Select>,
                      )}
              </FormItem>
              ...
              <Row gutter={0}>
                <Col span={12}>
                  <FormItem key="plan_working_hours" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.PlanWorkingHours' })}>
                  {form.getFieldDecorator('plan_working_hours', {
                    rules: [
                      {
                        required: true,
                        message: formatMessage({ id: 'project.NewCreateForm.AddItem.PlanWorkingHour.ValidationEmpty' }),
                      },
                      {
                        pattern: /^[0-9]*[1-9][0-9]*$/,
                        message: formatMessage({ id: 'project.NewCreateForm.AddItem.PlanWorkingHour.ValidationPositive' }),
                      },
                    ],
                    initialValue: formVals.plan_working_hours,
                  })(<Input onChange={this.onPlanWorkingHourChange} 
                      placeholder={formatMessage({ id: 'project.SearchInputTips' })} suffix="H" />)}
                  </FormItem>

                  </Col>
                  <Col span={12}>
                <FormItem key="hour_rate" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.RMBPerHour' })}>
              ...
              <Row gutter={0}>
                <Col span={12}>
                <FormItem key="plan_budget" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.PlanBudget' })}>
                ...
                <Col span={12}>
                <FormItem key="sales_order_volume" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.SalesOrderVolume' })}>
          ...
          <Row gutter={0}>
            <Col span={12}>
              <FormItem key="project_run_status" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.ProjectStatus' })}>
                    {form.getFieldDecorator('project_run_status' , {initialValue: formVals.project_run_status_id+"",})(
                          <Select placeholder={formatMessage({ id: 'project.SearchSelectTips' })} style={{width: '100%' }}>
          ...
          <Row gutter={0}>
          <Col span={12}>
                <FormItem key="start_time" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.StartTime' })}>
           ...
           <Col span={12}>
              <FormItem key="end_time" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.EndTime' })}>
            ...
            <Col span={12}>
            <FormItem key="update_time" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.UpdateTime' })}>
          ...
          <Row gutter={0}>
              <Col span={24} pull={0}>
                  <FormItem key="support_reason" {...this.formLayout} label={"Comments"}>
                    ...
                    })(<TextArea rows={2} autosize={{minRows: 2, maxRows: 4}} placeholder={formatMessage({ id: 'project.SearchInputTips' })} />)}
                    </FormItem>
                </Col>
          </Row>
          ...
          <Row gutter={0}>
            <Col span={12}>
              <FormItem key="engineer" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.Engineers' })}>
                  {form.getFieldDecorator('engineer', {
                    initialValue: formVals.arrEngineerId,
                    rules: [{ required: true,message:formatMessage({ id: 'project.NewCreateForm.Column.Engineers.RulesMessage' })}],
                  })(
                    <Select showSearch mode="multiple" placeholder={formatMessage({ id: 'project.NewCreateForm.Column.Apc.placeholder' })} optionFilterProp="children" style={{width: '100%' }}>
                      <OptGroup label="Sales">
                                  {salesOptions}
                                </OptGroup>
                                <OptGroup label="BD">
            ...
            <Col span={12}>
              <FormItem key="bd" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.Bd' })}>
                  {form.getFieldDecorator('bd', {
                    initialValue: formVals.arrBdId,
                    rules: [{ required: true,message:formatMessage({ id: 'project.NewCreateForm.Column.Bd.RulesMessage' })}],
                  })(
                    <Select showSearch mode="multiple" placeholder={formatMessage({ id: 'project.NewCreateForm.Column.Bd.placeholder' })} optionFilterProp="children" style={{width: '100%' }}>
                      {bdOptions}
                    </Select>,
                  )}
                </FormItem>
          ...
          <Row gutter={0}>
            <Col span={12}>
              <FormItem key="pss" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.Pss' })}>
                  {form.getFieldDecorator('pss', {
                    initialValue: formVals.arrPssId,
                  })(
                    <Select showSearch mode="multiple" placeholder={formatMessage({ id: 'project.NewCreateForm.Column.Pss.placeholder' })} optionFilterProp="children" style={{width: '100%' }}>
                      {pssOptions}
                    </Select>,
                  )}
                </FormItem>
            ...
            <Col span={12}>
              <FormItem key="sales" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.Sales' })}>
                  {form.getFieldDecorator('sales', {
                    initialValue: formVals.arrSalesId,
                  })(
                    <Select showSearch mode="multiple" placeholder={formatMessage({ id: 'project.NewCreateForm.Column.Sales.placeholder' })} optionFilterProp="children" style={{width: '100%' }}>
                      {salesOptions}
                    </Select>,
                  )}
                </FormItem>
          ...
          <Row gutter={0} /*style={{border:"solid 1px #000"}}*/>
           <Col span={24} /*style={{border:"solid 1px #f00"}}*/>
              <FormItem {...this.formLayout}  /*label={formatMessage({ id: 'project.NewCreateForm.Column.Partner' })}*/>
              <Button type="dashed" onClick={this.add} style={{ width: '100%',marginLeft:"40%",backgroundColor:"#A5E1E1" }}>
