  ...
  render() {
    ...
    const groupOptions =
      typeof groupList == 'undefined'
        ? []
        : groupList.map(d => <Option key={d.group_id}>{d.group_name}</Option>);
    const roleOptions =
      typeof roleList == 'undefined'
        ? []
        : roleList.map(d => <Option key={d.role_id}>{d.role_name}</Option>);
    const postOptions =
      typeof postList == 'undefined'
        ? []
        : postList.map(d => <Option key={d.post_id}>{d.post_name}</Option>);
    ...
    return (
      ...
      >
        ...
        <FormItem
          ...
          wrapperCol={{ span: 13 }}
          label={formatMessage({
            id: 'Member.CreateForm.Column.Role',
