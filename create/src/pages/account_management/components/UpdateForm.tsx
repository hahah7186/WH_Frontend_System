...
class UpdateForm extends Component<UpdateFormProps, UpdateFormState> {
  ...
  renderContent = ( formVals: FormValsType) => {
    ...
    const { form,fiscalYearList,accountTypeList,apcList,bdList,pssList,salesList } = this.props;
    let fisInitialVal = -1;
    ...
    //   }
    // });
    const fiscalYearOptions = fiscalYearList.map(d => <Option key={d.fiscal_id}>{d.fiscal_year}</Option>);
    const accountTypeOptions = accountTypeList.map(d => <Option key={d.type_id}>{d.type_name}</Option>);
debugger
    ...
    return [
      <FormItem key="account_id" {...this.formLayout} label={formatMessage({ id: 'project.NewCreateForm.Column.ProjectNo' })} style={{display:"none"}}>
      {form.getFieldDecorator('account_id', {
        initialValue: formVals.account_id,
      })(<Input prefix={"NO."} disabled={true} style={{textAlign:"center"}}/>)}
      </FormItem>,
      <FormItem key="account_name" {...this.formLayout} label={formatMessage({ id: 'Account.ModifyModel.Column.AccountName' })}>
      ...
      <FormItem key="account_type_id" {...this.formLayout} label={formatMessage({ id: 'Account.ModifyModel.Column.AccountType' })}>
    ...
    <FormItem key="member_id" {...this.formLayout} label={formatMessage({ id: 'Account.ModifyModel.Column.AccountOwner' })}>
    {form.getFieldDecorator('member_id', {
      ...
      <FormItem key="fiscal_year" {...this.formLayout} label={formatMessage({ id: 'Account.ModifyModel.Column.FiscalYear' })}>
        {form.getFieldDecorator('fiscal_year', {
          // rules: [{ required: true, message: formatMessage({ id: 'customer.NewCreateCustomer.FormItem.ValidationTips' }), min: 1 }],
          initialValue: fisInitialVal+"",
        })(
          <Select placeholder={formatMessage({ id: 'customer.CustomerSelectTips' })} disabled style={{width: '100%' }}>
            {fiscalYearOptions}
          </Select>
        )}
      </FormItem>,
      <FormItem key="budget" {...this.formLayout} label={formatMessage({ id: 'Account.CreateModel.Column.Budget' })}>
     ...
     <FormItem key="sap_order" {...this.formLayout} label={formatMessage({ id: 'Account.CreateModel.Column.SAPOrder' })}>
     {form.getFieldDecorator('sap_order' , {initialValue: formVals.sap_order,})(
           <Input placeholder={formatMessage({ id: 'customer.CustomerInputTips' })} />                
         )}
      </FormItem>,
      <FormItem key="comments" {...this.formLayout} label={formatMessage({ id: 'Account.CreateModel.Column.Comments' })}>
